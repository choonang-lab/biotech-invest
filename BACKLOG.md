...
Area built (tier:3, "T3" badge; ATLAS_DATA.areas.aesthetics + aes1–aes5). Figures `pending` — research pass verifying (Botox/Allergan Aesthetics revenue, Galderma, neurotoxin/filler market); apply when returns. Distinctive lesson: the CASH-PAY CONSUMER model (unique in the Atlas). Deep-dive lessons evergreen prose, verified render (headings/callouts/lists, 0 console errors), ?v=61, sw CACHE btgym-v4.
